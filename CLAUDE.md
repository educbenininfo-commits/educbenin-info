...
**Cron strategy.** No `setInterval` loops — Next.js / Vercel doesn't keep long-lived processes. Background work runs as **Vercel Cron** routes under `app/api/cron/<name>/route.ts`, each gated by `Authorization: Bearer ${CRON_SECRET}`. The routes themselves were built assuming per-minute scheduling (`outbox-drain`, `email-queue-drain`), but **this project's actual [vercel.json](frontend/vercel.json) runs every cron once a day** (Vercel Hobby plan caps cron frequency at daily — the 6 jobs are staggered a few minutes apart within one UTC hour as a workaround). Concretely: `outbox-drain` 06:00 UTC, `email-queue-drain` 06:15 UTC, `verification-cleanup` 06:30 UTC, `order-expiration` 06:45 UTC, `webhook-log-purge`/`email-job-purge` 00:00 UTC. **This means anything relying only on these crons waits up to 24h** — unacceptable for a signup verification code, a password-reset link, or an admin invitation. Those three routes ([frontend/src/app/api/auth/signup/route.ts](frontend/src/app/api/auth/signup/route.ts), [frontend/src/app/api/auth/forgot-password/route.ts](frontend/src/app/api/auth/forgot-password/route.ts), [frontend/src/app/api/auth/resend-verification/route.ts](frontend/src/app/api/auth/resend-verification/route.ts), [frontend/src/app/api/admin/invites/route.ts](frontend/src/app/api/admin/invites/route.ts)) call `drainOutboxNow()` ([frontend/src/lib/server/outbox/drain-now.ts](frontend/src/lib/server/outbox/drain-now.ts)) right after `enqueueOutbox()` — a small best-effort, never-throwing immediate drain (reuses the same protected `drainOutbox()` + `EmailQueue.drainOne()` the crons call, just with a tiny batch size) so the email actually goes out within the request instead of waiting for the next cron tick; the daily cron remains the fallback if that immediate attempt fails. Two routes with explicit enumeration-resistance (`forgot-password`'s `CR-01` wall-clock floor, `signup`'s `D-22` timing parity) call `drainOutboxNow()` on **every** branch, including the no-op one, so the added latency doesn't reopen a timing side-channel. Payment-confirmation emails (`webhooks/bictorys`) deliberately stay on the daily cron only — less time-sensitive, and the protected webhook handler is not a place to bolt on new side-effect timing. If per-minute cron scheduling becomes available again (Vercel Pro, or a different scheduler), `vercel.json` should be restored to real per-minute/per-5-minute schedules and this synchronous-drain workaround can be reassessed. Multi-instance coordination still uses [frontend/src/lib/server/leader-lease.ts](frontend/src/lib/server/leader-lease.ts) Redis leases where two crons could collide. The Bictorys charge `CircuitBreaker` is still in-memory single-instance — replace with a Redis-backed variant for multi-pod prod (documented limitation).
