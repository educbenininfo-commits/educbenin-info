...
Utilisatrice affichée dans `.bo-user` sur toutes les pages back-office : avatar rond "CA", nom **"Chimène A. · Agent de traitement"**. Ce n'est pas dynamique dans le prototype — c'est une donnée d'exemple statique, identique partout, y compris sur l'écran "Paramètres" où le profil réel affiché diffère (voir section Paramètres) — ⚠️ **incohérence à noter** : le nom dans `.bo-user` ("Chimène A. · Agent de traitement") ne correspond pas exactement au nom complet du formulaire Paramètres ("Chimène Adjahoui" / e-mail `chimene@educbenin.info`), mais désigne manifestement la même personne.
...
- **Pied de page** (`.footer`) : fond `--prod-surface-2`, grille 4 colonnes (`1.4fr 1fr 1fr 1fr` desktop, 2 colonnes mobile) — bloc marque + description, "Plateforme" (3 liens), "Légal" (3 liens), "Contact" (WhatsApp Educ Bénin — lien non fonctionnel, texte seul ; pas d'e-mail de contact tant qu'aucune adresse professionnelle n'existe, un numéro de téléphone viendra s'ajouter ici une fois renseigné dans Paramètres). Ligne légale finale : *"© 2026 Educ Bénin — Cotonou, Bénin"* / *"Aucune affiliation avec la FSS ou l'UAC"*.
...
- Formulaire (`#stepper` + `.form-step`) : indicateur d'étapes horizontal (3 pastilles rondes 26px numérotées, reliées par des barres 1px) — pastille active : fond `--prod-primary` ; pastille complétée (`.done`) : fond `--prod-success` ; libellés visibles uniquement desktop (`display:none` sur mobile via `.stepper .lbl`).
  ...
  - **Étape 2** : deux champs Nom/Prénom côte à côte (`.row2`, 1 colonne en mobile), champ "Numéro WhatsApp" pleine largeur (composant `CountryPhoneInput` : indicatif+drapeau en menu déroulant pré-rempli selon le pays détecté du navigateur, formatage du numéro adapté au pays choisi, valeur stockée en E.164) avec indice "Canal utilisé pour tout le suivi de votre dossier.", puis champ "Nationalité" (composant `CountrySelect` : menu déroulant de tous les pays avec drapeau, même pré-remplissage par pays détecté).
...
- Champs étape 2 : "Nom" (placeholder "AMOUSSOU"), "Prénom" (placeholder "Koffi"), "Numéro WhatsApp" (indicatif pays + numéro, pré-rempli selon le pays détecté — plus limité au seul Bénin), "Nationalité" (menu déroulant pays + drapeau, pré-rempli selon le pays détecté, modifiable).
- Au-dessus de la zone de dépôt de fichier de l'étape 3 : mention de la taille maximale (5 Mo) + lien "Compresser un PDF trop volumineux →" vers ilovepdf.com (nouvel onglet).
- Validation : au clic sur "Continuer"/"Envoyer ma demande" si des champs obligatoires sont vides, ceux-ci sont surlignés en rouge (bordure `--prod-danger`) au lieu d'un seul message générique — comportement ajouté au-delà du prototype d'origine, qui ne validait rien.
...
- **Cas "Rejeté"** (état 0) : rendu complètement différent — une seule ligne, pastille rouge pleine, titre "Dossier rejeté" + badge rouge "À corriger", description : *"Motif : {motifRejet du dossier, ou "aucun motif communiqué."}"* + encart jaune fixe (ne varie pas avec le motif) : *"💬 Besoin d'en savoir plus ? Contactez-nous via WhatsApp en précisant le motif de votre préoccupation."* (texte mis à jour — l'ancien "Merci de nous transmettre les corrections nécessaires via WhatsApp." laissait entendre à tort qu'aucune clarification n'était possible sans passer par WhatsApp).
- **Messages de l'équipe Educ Bénin** (bloc ajouté sous la timeline, hors prototype d'origine) : si le dossier a au moins un commentaire de type `public` (posté depuis le back-office, onglet "Commentaires publics"), affiche la liste de ces commentaires (texte + horodatage relatif) sous un titre "Messages de l'équipe Educ Bénin" — **quels que soient l'étape et le statut du dossier, y compris "en attente du candidat" et "rejeté"**. Absent du prototype (qui ne prévoyait aucune remontée de commentaire côté candidat) ; ajouté suite à un bug signalé où un commentaire public posté à l'étape 2 n'apparaissait jamais côté candidat.
...
- **Colonne droite** : formulaire centré (`max-width:320px`) — titre "Connexion" (20px), champ "Adresse e-mail" (pré-rempli "agent@educbenin.info"), champ "Mot de passe" (type password, valeur factice "••••••••••"), bouton primaire pleine largeur "Se connecter", séparateur "ou" (`.divider`, ligne + texte centré), bouton Google (`.g-btn` — logo Google SVG 4 couleurs officiel + texte "Se connecter avec Google"), puis note "Mot de passe oublié ? Contactez un administrateur."

### Content
Voir ci-dessus — tous les textes sont déjà cités intégralement. Valeurs de démonstration : e-mail `agent@educbenin.info`, mot de passe masqué de 10 points.
...
- **Modale** (`#dossierOverlay` → `.modal`, voir Fondations pour la structure générale) :
  ...
  - **Grille d'actions** (`.actions-grid`, flex-wrap) — le prototype d'origine prévoyait exactement 4 boutons ; le produit réel en compte davantage, ajoutés pour couvrir des besoins non traités par le prototype : "Modifier" (désormais fonctionnel, voir plus bas) · "Envoyer"/"Renvoyer le formulaire d'authentification" (libellé variable, voir logique ci-dessous — **ne disparaît plus une fois envoyé**, contrairement au prototype) · "Voir le formulaire d'authentification" (bouton séparé, visible dès qu'une soumission existe, indépendant de l'état d'envoi) · "Renvoyer pour correction" (étapes 1 et 2, régénère un lien préexistant pré-rempli et pose l'indicateur "Dossier MAJ") · "Voir la fiche d'inscription" · "Voir le récépissé" (libellé variable). Chaque bouton peut être désactivé (`.is-disabled`) selon l'étape. Un pill supplémentaire "Dossier MAJ" apparaît à côté du pill de statut quand une correction a été demandée et n'a pas encore été re-soumise (`correctionRequestedAt` non nul) ; un pill "En attente du candidat" peut aussi apparaître en plus du bouton d'envoi (jamais à sa place).
  - **Aperçu formulaire d'authentification** (`#authPreview`, `.preview-box`, masqué par défaut) — apparaît sous la grille d'actions quand on clique "Voir le formulaire d'authentification". Ce n'est PAS le formulaire de demande initiale (les 3 étapes de la page Accompagnement) — c'est la relecture, côté back-office, des réponses soumises par le médecin sur le **formulaire externe d'authentification de diplôme** qui lui est envoyé par WhatsApp une fois à cette étape. Structure exacte, dans cet ordre :
    ...
    4. Section "DIPLÔME DU BACCALAURÉAT" → `<dl>` : Institution étatique délivrant ce diplôme (libellé mis à jour — anciennement "Institution (établissement étatique)"), E-mail institution, Année d'obtention, Pays d'obtention, Adresse institution.
    5. Section "DIPLÔME DU DOCTORAT" → même structure que le Bac.
    Le vrai formulaire externe (voir 16-formulaire-authentification-diplome.md) affiche en plus, avant ces deux sections : un bandeau "⚠️ Les nom et prénom saisis ci-dessous doivent être strictement conformes à ceux inscrits sur votre diplôme." et, sous le titre de chaque section diplôme, la mention "Si votre diplôme n'est pas rédigé en français, faites-le traduire par le ministère des Affaires étrangères de votre pays avant de le soumettre." ainsi qu'une case à cocher "Cocher cette case si l'original de votre diplôme n'est pas en français, afin d'ajouter les documents traduits." qui, une fois cochée, révèle deux champs d'upload supplémentaires (traduction du Bac, traduction du Doctorat) — donc jusqu'à **4 fichiers PDF** au lieu de 2 (`diplomaBacUrl`/`diplomaDoctoratUrl`/`diplomaBacTranslatedUrl`/`diplomaDoctoratTranslatedUrl`), chacun avec la mention de la taille max (5 Mo) et un lien de compression ilovepdf.com.
  ...
  - **Pied de modale** (`.modal-foot`) : "Rejeter le dossier" (danger outline, gauche) · "Restaurer vers Dossiers reçus" (outline, milieu) · "← Étape précédente" (outline, ajouté — actif pour `2 <= stage <= 5`, décrémente l'étape d'un cran) · "Faire passer à l'étape suivante →" (primaire, droite).
  - **Onglet "Historique"** (4ᵉ onglet, ajouté au-delà du prototype, à côté de Paiement/Commentaires publics/internes) : liste chronologique des `AdminAction` du dossier (action, auteur, date, résumé du diff avant/après) — expose en lecture ce que `logAdminAction` enregistre déjà à chaque mutation (paiement, rejet, avancement, envoi/renvoi de formulaire, ajout de commentaire, modification).
  - **Bloc "Informations de la demande"** (ajouté, absent du prototype) : nom/prénom/WhatsApp/nationalité/spécialités tels que soumis à la demande initiale, avec un bouton "Télécharger en PDF" à côté (comme pour l'aperçu du formulaire d'authentification) — objectif : copier-coller rapide, pas un document soigné.
...
**3. Logique du bouton d'envoi/renvoi du formulaire d'authentification — `authSendButtonState(d)` (renommée depuis `authButtonState`, comportement revu : le produit réel ne désactive plus jamais ce bouton une fois soumis, contrairement au prototype)** :
   ...
   - Si `stage === 2` **et** `!d.authSentAt` (jamais envoyé) : bouton **activé**, libellé **"Envoyer le formulaire d'authentification"**.
   - Si `stage === 2` et `d.authSentAt` (déjà envoyé, soumis ou non) : bouton **reste activé**, libellé **"Renvoyer le formulaire d'authentification"** — cliquer régénère le token, remet `authSubmittedAt` à `null` (rouvre la soumission pour le candidat) et **conserve** les réponses déjà soumises comme pré-remplissage du formulaire public. Un pill séparé "En attente du candidat" s'affiche **en plus** (jamais à la place du bouton) tant que `authSentAt` est posé et `authSubmittedAt` ne l'est pas.
   - **Bouton "Voir le formulaire d'authentification"** (séparé du bouton d'envoi, ajouté) : visible dès qu'une soumission existe (`authFormData` non nul), indépendamment de l'état d'envoi — bascule l'affichage de `#authPreview` comme avant.
   - **Bouton "Renvoyer pour correction"** (ajouté, étapes 1 et 2) : régénère le lien pertinent (formulaire de demande à l'étape 1 via un nouveau token `correctionToken` et un nouvel écran public `/corriger-ma-demande/[token]`, ou formulaire d'authentification à l'étape 2 via le même mécanisme que "Renvoyer" ci-dessus) pré-rempli avec les informations déjà connues, **sans jamais créer un nouveau dossier** — met à jour le même dossier et pose `correctionRequestedAt`, affiché comme pill "Dossier MAJ" jusqu'à la nouvelle soumission du candidat.
...
**6. Bouton "Modifier"** : toujours activé, tous les stades, libellé fixe, info-bulle "Corriger une information saisie par le candidat." — **désormais fonctionnel** (le prototype n'implémentait aucune action de clic) : ouvre un panneau d'édition inline (nom/prénom/WhatsApp/nationalité, plus e-mail/téléphone du formulaire d'authentification si soumis), "Enregistrer" appelle `PATCH /api/admin/dossiers/[id]/edit`, qui journalise un diff avant/après consultable dans l'onglet "Historique".
...
- Panneau unique (`max-width:420px`) : champ "Nom complet" (valeur "Chimène Adjahoui"), champ "E-mail" (valeur "chimene@educbenin.info"), champ "Nouveau mot de passe" (vide, placeholder "••••••••"), bouton outline pleine largeur "Enregistrer les modifications", puis bouton danger-outline pleine largeur "Se déconnecter" (`data-goto="login"`).
